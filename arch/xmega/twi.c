...
static uint8_t twi_nb_bytes_to_write = 0;
static uint8_t *twi_write_data;

/**
 * RXACK : When read as zero, the most recent acknowledge bit from the slave was ACK,
 * and when read as one the most recent acknowledge bit was NACK
 * */
void
xmega_twi_master_write_handler (TWI_t *twi)
{
  ...
  if (twi->MASTER.STATUS & TWI_MASTER_RXACK_bm)
    {
      twi->MASTER.CTRLC = TWI_MASTER_CMD_STOP_gc; // stop transaction
    }
...
{
  /* When smart mode is enabled, the acknowledge action, as set by the ACKACT bit in
  ...
  twi->MASTER.STATUS = TWI_MASTER_BUSSTATE_IDLE_gc; // indicate the current TWI bus state
#if F_CPU == 32000000UL
  twi->MASTER.BAUD = 155; //(32000 / (2 * freq)) - 5; // 155 to 100kHz - 35 to 400kHz
#endif
}
 ...
 */
void
xmega_twi_write (TWI_t *twi, uint8_t slave_address, uint8_t *write_data,
		 uint8_t nb_bytes_to_write)
{
  ...
  // if slave returned NACK or did not reply at all : send address until slave returns ACK
