...
pin_setup (void)
{
  ...
  PORTE.PIN4CTRL = PORT_ISC_LEVEL_gc | PORT_OPC_PULLUP_gc; // A : QDPH0 - D0
  PORTE.PIN5CTRL = PORT_ISC_LEVEL_gc | PORT_OPC_PULLUP_gc; // B : QDPH90 - D1

  /* PORTF encoder counter (PF0, PF1, PF4 and PF5) as input pin, Set QDPH0 and QDPH1 sensing level
   * index not used here
   */
  PORTF.DIRCLR = PIN0_bm;
  PORTF.DIRCLR = PIN1_bm;
  PORTF.PIN0CTRL = PORT_ISC_LEVEL_gc | PORT_OPC_PULLUP_gc; // A : QDPH0 - D0
  PORTF.PIN1CTRL = PORT_ISC_LEVEL_gc | PORT_OPC_PULLUP_gc; // B : QDPH90 - D1
  PORTF.DIRCLR = PIN4_bm;
  PORTF.DIRCLR = PIN5_bm;
  PORTF.PIN4CTRL = PORT_ISC_LEVEL_gc | PORT_OPC_PULLUP_gc; // A : QDPH0 - D0
  PORTF.PIN5CTRL = PORT_ISC_LEVEL_gc | PORT_OPC_PULLUP_gc; // B : QDPH90 - D1
...
qdec_setup (void)
{
  ...
  EVSYS.CH0MUX = EVSYS_CHMUX_PORTE_PIN4_gc; // A & B inputs to quadrature decoder
  //EVSYS.CH0MUX = EVSYS_CHMUX_PORTE_PIN6_gc; // index input to quadrature decoder
  ...
  /* Configure event channel x assign to pin x */
  EVSYS.CH2MUX = EVSYS_CHMUX_PORTF_PIN0_gc; // A & B inputs to quadrature decoder
  //EVSYS.CH2MUX = EVSYS_CHMUX_PORTF_PIN2_gc; // index input to quadrature decoder
  ...
  /* Configure event channel x assign to pin x */
  /*EVSYS.CH4MUX = EVSYS_CHMUX_PORTF_PIN4_gc; // A & B inputs to quadrature decoder
   EVSYS.CH4MUX = EVSYS_CHMUX_PORTF_PIN6_gc; // index input to quadrature decoder
   EVSYS.CH4CTRL = EVSYS_QDEN_bm
   | EVSYS_DIGFILT_2SAMPLES_gc;
   EVSYS.CH1CTRL = EVSYS_DIGFILT_2SAMPLES_gc;
   xmega_timer_1_qdec_mode_setup (&TCF1, TC_EVSEL_CH4_gc, 512);*/
}

volatile uint8_t controller_flag = 0;
...
setup (void)
{
  ...
  xmega_timer_0_pwm_mode_setup (&TCE0, 200, TC_CLKSEL_DIV8_gc); /* 200 pour 20KHz -100 pour pour 40 KHz */
...
main (void)
{
  uint8_t all_irs[] =
    { 2, 3, 4, 5, 6, 7 };
  setup ();
  xmega_adc_read (&ADCA, 0); // start first conversion
